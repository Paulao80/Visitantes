    ...
    public class RelatorioController : Controller
    {
        ...
        public ActionResult Index(DateTime? DateIni, DateTime? DateEnd, int? Tipo)
        {
            if (DateIni != null && DateEnd != null && Tipo != null)
            {
                ...
                if (Tipo == 0)
                {
                    arquivo = _ReportService.GerarSaidaTransporte((DateTime)DateIni, dtEnd).ToArray();
                    return RedirectToAction("Exibir");
                }
                else if (Tipo == 1)
                {
                    arquivo = _ReportService.GerarEntradaTransporte((DateTime)DateIni, dtEnd).ToArray();
                    return RedirectToAction("Exibir");
                }
                else if(Tipo == 2)
                {
                    arquivo = _ReportService.GerarAllTransporte((DateTime)DateIni, dtEnd).ToArray();
                    return RedirectToAction("Exibir");
                }
            }
